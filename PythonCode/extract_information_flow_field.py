...
def determine_optical_flow(prev_bgr, bgr, graphics= False):
    ...
    ###########################################################################
    x = np.array(points_old[:,0])
    y = np.array(points_old[:,1])
    
    u = np.array(flow_vectors[:,0])
    v = np.array(flow_vectors[:,1])
    
    if u != [] and v != []:
        xfoe = 80
        yfoe = 150
        ttc1 = abs(x-xfoe)/abs(u)
        ttc2 = abs(y-yfoe)/abs(v)
    ...
    if(graphics):
        ...
        for p in range(n_points):
         ...
         ################################################################
            if u != [] and v != []:
                cv2.circle(im, tuple(points_old[np.argmin(ttc1),:]), 5,(0,0,255),-1)
#                cv2.circle(im, tuple(points_old[np.argmin(ttc2),:]), 5,(0,0,255),-1)
...
def estimate_linear_flow_field(points_old, flow_vectors, RANSAC=True, n_iterations=100, error_threshold=5.):
...
def extract_flow_information(image_dir_name = 'cyberzoo_ex/', image_type = 'jpg', verbose=True, graphics = True, flow_graphics = False):
    
    # get the image names from the directory:
    image_names = sorted(os.listdir(image_dir_name))
    ...
    prev_bgr = cv2.imread(os.path.join(image_dir_name,image_names[0]))
    
    for im in np.arange(1, n_images, 1):
        
        bgr = cv2.imread(os.path.join(image_dir_name,image_names[im]))
        
        if(im > 0):
            
            t_before = time.time()
            # determine optical flow:
            points_old, points_new, flow_vectors = determine_optical_flow(prev_bgr, bgr, flow_graphics);
            # do stuff
